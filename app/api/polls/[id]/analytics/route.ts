...
export async function GET(
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Access denied - You can only view analytics for your own polls",
        },
        { status: 403 },
      );
    }
    ...
    const recentVotes = votes.filter(
      (vote) => new Date(vote.created_at).getTime() > last24Hours,
    );

    const votesByHour = recentVotes.reduce(
      (acc, vote) => {
    ...
    const averageVotesPerDay =
      pollDurationDays > 0 ? totalVotes / pollDurationDays : totalVotes;
    const peakVotingHour =
      Object.entries(votesByHour).sort(([, a], [, b]) => b - a)[0]?.[0] || null;
